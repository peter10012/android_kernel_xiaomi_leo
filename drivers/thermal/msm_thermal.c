...
#define UPDATE_THRESHOLD_SET(_val, _trip)                \
	do                                                   \
	{                                                    \
...
static void msm_thermal_update_freq(bool is_shutdown, bool mitigate)
{
	...
	for_each_possible_cpu(cpu)
	{
		if (msm_thermal_info.freq_mitig_control_mask & BIT(cpu))
		{
			...
			uint32_t mitigation_freq = (mitigate) ? get_core_min_freq(cpu) : UINT_MAX;
...
static void update_cpu_freq(int cpu)
{
	...
	if (cpu_online(cpu))
	{
		if ((cpumask_intersects(&mask, &throttling_mask)) && (cpus[cpu].limited_max_freq >= get_core_max_freq(cpu)))
		{
...
static int *__init get_sync_cluster(struct device *dev, int *cnt)
{
	...
	if (!of_get_property(dev->of_node, key, &cluster_cnt) || cluster_cnt <= 0 || !core_ptr)
		return NULL;
										...
										cpumask_t *cluster_cpus)
{
	...
	if (!of_get_property(dev->of_node, key, &cluster_cnt) || cluster_cnt <= 0)
	{
...
static int __ref init_cluster_freq_table(void)
{
														  ...
														  policy = NULL, freq_table_ptr = NULL)
	{
		...
		while (freq_table_ptr[table_len].frequency != CPUFREQ_TABLE_END)
			table_len++;

		cluster_ptr->freq_idx_low = 0;
		cluster_ptr->freq_idx_high = cluster_ptr->freq_idx =
			table_len - 1;
		if (cluster_ptr->freq_idx_high < 0 || (cluster_ptr->freq_idx_high < cluster_ptr->freq_idx_low))
		{
...
static void update_cluster_freq(void)
{
														  ...
														  online_cpu = -1, max = UINT_MAX, min = 0)
	{
		...
		*/
		...
		if (cluster_ptr->limited_max_freq == max && cluster_ptr->limited_min_freq == min)
			continue;
	...
	for (; _cluster < core_ptr->entity_count; _cluster++)
	{
		...
		if (mitigate)
			freq_idx = max_t(int, cluster_ptr->freq_idx_low,
							 (cluster_ptr->freq_idx - msm_thermal_info.bootup_freq_step));
		else
		...
		for_each_cpu_mask(_cpu, cluster_ptr->cluster_cores)
		{
			if (!(msm_thermal_info.bootup_freq_control_mask & BIT(_cpu)))
				continue;
			pr_info("Limiting CPU%d max frequency to %u. Temp:%ld\n", _cpu, cluster_ptr->freq_table[freq_idx].frequency, temp);
...
static int update_cpu_min_freq_all(uint32_t min)
{
	...
	if (core_ptr)
	{
		for (; _cluster < core_ptr->entity_count; _cluster++)
		{
			...
			valid_table = true;
			min = min(min,
					  cluster_ptr->freq_table[cluster_ptr->freq_idx_high].frequency);
		}
		if (!valid_table)
			return ret;
	}
...
static ssize_t store_phase_request(const char *buf, size_t count, bool is_cx)
{
	...
	enum msm_thermal_phase_ctrl phase_req = (is_cx) ? MSM_CX_PHASE_CTRL : MSM_GFX_PHASE_CTRL;
	...
	for (i = 0; i < thresh[MSM_VDD_MX_RESTRICTION].thresh_ct; i++)
	{
		...
		if (ret)
		{
			pr_err("Unable to read TSENS sensor:%d, err:%d\n",
				   thresh[MSM_VDD_MX_RESTRICTION].thresh_list[i].sensor_id, ret);
			dis_cnt++;
			continue;
		}
	...
	while (!kthread_should_stop())
	{
		...
		for_each_possible_cpu(cpu)
		{
				...
				cpus[cpu].hotplug_thresh_clear)
			{
				...
				if (cpus[cpu].offline && !IS_LOW_THRESHOLD_SET(ret))
					cpus[cpu].offline = 0;
		...
		if (devices && devices->hotplug_dev)
		{
			...
			mutex_lock(&devices->hotplug_dev->clnt_lock);
			for_each_cpu_mask(cpu,
							  devices->hotplug_dev->active_req.offline_mask) if (mask & BIT(cpu))
...
static __ref int do_freq_mitigation(void *data)
{
	...
	while (!kthread_should_stop())
	{
		...
		for_each_possible_cpu(cpu)
		{
			max_freq_req = (cpus[cpu].max_freq) ? msm_thermal_info.freq_limit : UINT_MAX;
			...
			if ((max_freq_req == cpus[cpu].limited_max_freq) && (min_freq_req ==
																 cpus[cpu].limited_min_freq))
		...
		reset_threshold:
				...
				cpus[cpu].freq_thresh_clear)
			{
				...
				if (cpus[cpu].max_freq && !IS_LOW_THRESHOLD_SET(ret))
				{
...
static void cx_phase_ctrl_notify(struct therm_threshold *trig_thresh)
{
	...
	pr_debug("sensor:%d reached %s thresh for CX\n",
			 ...
			 (trig_thresh->trip_triggered == THERMAL_TRIP_CONFIGURABLE_HI) ? "hot critical" : "warm");
	...
	if ((cx_sens_status && (curr_cx_band == MSM_HOT_CRITICAL)) ||
		...
		goto cx_phase_unlock_exit;
	ret = send_temperature_band(MSM_CX_PHASE_CTRL, (cx_sens_status) ? MSM_HOT_CRITICAL : MSM_WARM);
	if (!ret)
...
static void vdd_restriction_notify(struct therm_threshold *trig_thresh)
{
	...
	pr_debug("sensor:%d reached %s thresh for Vdd restriction\n",
			 tsens_id_map[trig_thresh->sensor_id],
			 (trig_thresh->trip_triggered == THERMAL_TRIP_CONFIGURABLE_HI) ? "high" : "low");
	switch (trig_thresh->trip_triggered)
	{
	case THERMAL_TRIP_CONFIGURABLE_HI:
	...
	if (ret)
	{
		pr_err("%s vdd rstr votlage for all failed\n",
			   (vdd_sens_status) ? "Enable" : "Disable");
		goto unlock_and_exit;
	}
...
static void ocr_notify(struct therm_threshold *trig_thresh)
{
	...
	ret = ocr_set_mode_all(ocr_sens_status ? OPTIMUM_CURRENT_MAX : OPTIMUM_CURRENT_MIN);
	if (ret)
	{
		pr_err("%s Optimum current mode for all failed. err:%d\n",
			   (ocr_sens_status) ? "Enable" : "Disable", ret);
		goto unlock_and_exit;
	}
							  ...
							  void (*callback)(struct therm_threshold *))
{
	...
	if (!dev || !callback || !thresh_inp || sensor_id == -ENODEV)
	{
		pr_err("Invalid input\n");
		ret = -EINVAL;
	...
	mutex_lock(&threshold_mutex);
	thresh_inp->thresh_ct = (sensor_id == MONITOR_ALL_TSENS) ? max_tsens_num : 1;
	thresh_inp->thresh_triggered = false;
	thresh_inp->thresh_list = kzalloc(sizeof(struct therm_threshold) *
										  thresh_inp->thresh_ct,
									  GFP_KERNEL);
	if (!thresh_inp->thresh_list)
	{
										 ...
										 struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	for_each_possible_cpu(cpu)
	{
		...
		pr_debug("\"%s\"(PID:%i) requests %s CPU%d.\n", current->comm,
				 current->pid, (cpus[cpu].user_offline) ? "offline" : "online", cpu);
	}
...
int msm_thermal_pre_init(struct device *dev)
{
	...
	if (!thresh)
	{
		...
		memset(thresh, 0, sizeof(struct threshold_info) * MSM_LIST_MAX_NR);
	}
pre_init_exit:
	return ret;
}
								...
								struct platform_device *pdev)
{
	...
	ret = sensor_mgr_init_threshold(&pdev->dev,
									...
									data->gfx_sensor,
									data->gfx_phase_warm_temp_degC, data->gfx_phase_warm_temp_degC - data->gfx_phase_warm_temp_hyst_degC,
...
probe_gfx_crit:
	...
	ret = sensor_mgr_init_threshold(&pdev->dev,
									...
									data->gfx_sensor,
									data->gfx_phase_hot_temp_degC, data->gfx_phase_hot_temp_degC - data->gfx_phase_hot_temp_hyst_degC,
									gfx_phase_ctrl_notify);
	if (ret)
	{
							   ...
							   struct platform_device *pdev)
{
	...
	ret = sensor_mgr_init_threshold(&pdev->dev,
									...
									MONITOR_ALL_TSENS,
									data->cx_phase_hot_temp_degC, data->cx_phase_hot_temp_degC - data->cx_phase_hot_temp_hyst_degC,
