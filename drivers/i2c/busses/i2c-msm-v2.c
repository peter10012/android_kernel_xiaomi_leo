...
i2c_msm_qup_xfer_init_reset_state(struct i2c_msm_ctrl *ctrl)
{
	...
	io_modes_reg &=
		~(QUP_INPUT_MODE | QUP_OUTPUT_MODE | QUP_PACK_EN | QUP_UNPACK_EN | QUP_OUTPUT_BIT_SHIFT_EN);
	io_modes_reg |=
	 ...
	 */
	op_mask = (xfer->mode_id == I2C_MSM_XFER_MODE_DMA) ? (QUP_INPUT_SERVICE_MASK | QUP_OUTPUT_SERVICE_MASK) : 0;
									...
									int ht_div, int noise_rjct_scl, int noise_rjct_sda)
{
	...
	ctrl->mstr_clk_ctl = (reg_val & (~0xff07ff)) | ((ht_div & 0xff) << 16) | (fs_div & 0xff);
...
static irqreturn_t i2c_msm_qup_isr(int irq, void *devid)
{
	...
	if (xfer->mode_id == I2C_MSM_XFER_MODE_BLOCK)
	{
		...
		if (qup_op & QUP_OUTPUT_SERVICE_FLAG)
		{
			...
			if ((qup_op & blk->complete_mask) == blk->complete_mask)
			{
				log_event = true;
				signal_complete = true;
			}
...
static void i2c_msm_qup_init(struct i2c_msm_ctrl *ctrl)
{
	...
	writel_relaxed(QUP_OUTPUT_OVER_RUN_ERR_EN | QUP_INPUT_UNDER_RUN_ERR_EN | QUP_OUTPUT_UNDER_RUN_ERR_EN | QUP_INPUT_OVER_RUN_ERR_EN,
...
{
	struct pinctrl_state *pin_state = pinctrl_lookup_state(ctrl->rsrcs.pinctrl, name);
